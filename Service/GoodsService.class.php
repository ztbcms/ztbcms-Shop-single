...
class GoodsService extends BaseService {
     ...
     * @return array
     */
    public static function getGoodsList($where, $catid, $order, $onsale, $page = 1, $limit = 20) {
    ...
    public static function getGoodsInfo($goods_id) {
        ...
        $goods_images_list = M(GoodsService::GOODS_IMAGES_TABLE_NAME)->where("goods_id = '%d'",
            $goods_id)->select(); // 商品 图册
        ...
        $goods_attr_list = M(GoodsService::GOODS_ATTR_TABLE_NAME)->where("goods_id = '%d'",
            $goods_id)->select(); // 查询商品属性表
...
//        $post_goods_attr = array();
     ...
     * @return array
     */
     public static function getGoodNum($goods_id, $key) {
        if (!empty($key)) {
            $data = M("ShopSpecGoodsPrice")->where("goods_id = $goods_id and `key` = '$key'")->getField('store_count');
            return self::createReturn(true, $data, '获取成功');
        } else {
